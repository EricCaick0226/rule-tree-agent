...
class RowFirstPipelineTests(unittest.TestCase):
    ...
    def test_run_step_routes_deterministic_row_first_steps(self) -> None:
        ...
        with patch("src.pipeline.agent_executor.prefill_rows_from_reference_library", return_value=state) as prefill:
            self.assertIs(_run_step("prefill_rows_from_reference_library", state, "outputs", object()), state)
        prefill.assert_called_once_with(state)
    ...
    def test_row_extraction_prompt_requires_all_rows_and_continuation_inheritance(self) -> None:
        ...
        self.assertIn("structure_context", prompt)
        self.assertIn("版式上下文", prompt)
        self.assertIn("不是业务答案", prompt)
